    ...
    <li className="relative">
      <div className="group">
        <button
          ...
          className="flex items-center gap-1 w-full lg:w-auto lg:px-3 py-2 dark:text-white text-gray-600 hover:text-gray-900"
        >
          <span>{title}</span>
          <svg
              ...
              {children &&
                children.map((item) => (
                  <Link href={item.path} key={item.title} legacyBehavior>
                    <a className="py-1  hover:text-gray-900 dark:text-white text-black text-md ">
                      {item.title}
                    </a>
                  </Link>
                ))}
